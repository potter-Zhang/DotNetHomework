...
namespace WebCrawler
{
    ...
    internal class Crawler
    {
        ...
        private string getComponentStr = @"^(?<site>(?<protocal>https?)://(?<host>[\w\d.-]+)(:\d+)?($|/))(\w+/)*(?<file>[^#?]*)";
        ...
        {
            Match matches = new Regex(getComponentStr).Match(url);
            comp.protocol = matches.Groups["protocal"].Value;
            comp.hostName = matches.Groups["host"].Value;
            comp.filePath = matches.Groups["file"].Value;
        ...
        public void Download_and_parse(string current)
        {
            count++;
        ...
        {
            sb.Clear();
            List<Action> task = new List<Action>();
            ...
            while (count <= 10)
            {
                ...
                foreach (string url in urls.Keys)
                {
                    ...
                    task.Add(() => Download_and_parse(url));
                }
                if (task.Count == 0)
                    break;
                Parallel.Invoke(task.ToArray());
                task.Clear();
            }
        ...
        {
            string valid = ".(html?|aspx|jsp|php)$|^[^.]*$";
            var matches = new Regex(valid).Matches(url);
            var hostfilter = new Regex(getComponentStr).Match(url);

            return matches.Count > 0 && hostfilter.Groups["host"].Value == comp.hostName;
            //int k = url.IndexOf(';');
            //return k == -1;
